...
import com.example.youandi.auth.MenteeLoginActivity
...
class mProfileEditActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_mprofile_edit)
        ...
        editBtn.setOnClickListener {
            if(imgUri==null){
                Toast.makeText(this,"프로필 확인 필요", Toast.LENGTH_LONG).show()
            }else{
                val path = createCopyAndReturnRealPath(imgUri!!)
                val file = File(path)

                var requestBody : RequestBody = RequestBody.create("image/*".toMediaTypeOrNull(),file)
                var body : MultipartBody.Part = MultipartBody.Part.createFormData("uploadProfile","test",requestBody)
                ...
                //creating our api
                var server = retrofit.create(mFileEditInterface::class.java)
                ...
                server.modify(userIdRequestBody,body)	// body 가 Multipart.Part
                    .enqueue(object : Callback<String> {
                        ...
                        override fun onResponse(
                            ...
                            myClass.user_profile = imgUri.toString()
                        }
                    })
